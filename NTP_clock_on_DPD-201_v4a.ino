...
// ver.1 - added DST (Daylight Saving Time) or Summer Time control using local webpage
// ver.1a - added EEPROM store for TimeZone and DST
// ver.1a1 - clear the sketch
// ver.2 - add WiFiManager by tzapu
// ver.3 - added TimeZone webcontrol
// ver.3a - clear webpage (autoredirect to base page)
// ver.4 - added DHT sensor with "DHTesp.h" library
// ver.4a - added DHT display webcontrol and autoclear info at DHT read error
...
#define DHTpin 12  // D6 on R1 Mini 
...
void setup () {
 ...
 // Autodetect is not working reliable, don't use the following line
  // dht.setup(DHTpin);
  // use this instead: 
  dht.setup(DHTpin, DHTesp::DHT22); // Connect DHT sensor to GPIO xx
...
EEPROM.write(adresa + 2, 0);  // 0 => no DHT sensor, 1 => DHt sensor
...
if (realDHT == 1) Serial.println("yes");
      ...
      if (client.available()) {             // if there's bytes to read from the client,
        ...
        if (c == '\n') {                    // if the byte is a newline character
          ...
          { // If the current line is blank, you got two newline characters in a row. That's the end of the client HTTP request, so send a response:
            ...
            if (header.indexOf("GET /vara/on") >= 0) { // If the user clicked the summer's on button
            ...
            else if (header.indexOf("GET /vara/off") >= 0) { // If the user clicked the winter's on button
            ...
            if (header.indexOf("GET /TZminus") >= 0) { // If the user clicked the TimeZone change button
             ...
             if (header.indexOf("GET /DHTsensor") >= 0) { // If the user clicked the TimeZone change button
              realDHT = realDHT + 1;
              realDHT = realDHT%2;
              if (realDHT == 0) Serial.println("DHT sensor was removed !");
              else Serial.println("DHT sensor was added !");
              client.println("<meta http-equiv='Refresh' content=0;url=//" + WiFi.localIP().toString()+ ":80/>");
            } 
     /*       
...
//Get a time structure
  ...
  zz = timeClient.getDay();
            ...
            client.println("<br><br>");
             if (realDHT == 0) 
             {
                 client.println("<h2 class=\"text-center\">no DHT22 sensor !</h2>");
                 client.println("<p class=\"text-center\"><a href=\"/DHTsensor\"><button class=\"btn btn-sm btn-danger\">activate sensor</button></a></p>");
             }
             else
             {
                 ...
                 client.println("<p class=\"text-center\"><a href=\"/DHTsensor\"><button class=\"btn btn-success btn-sm\">deactivate sensor</button></a></p>");
             }
...
if (realDHT0 != realDHT)
{
EEPROM.write(adresa + 2, realDHT);  // 1 => DHT sensor, 0 => no DHT sensor 
EEPROM.commit();    //Store data to EEPROM 
Serial.print("Write in EEPROM: realDHT = ");
Serial.println(realDHT);
if (realDHT == 1)
{ 
citiresenzor();
tpcitire = millis();
...
TIMEZONE = (float)diferenta[timezone0];  // convert in hours
timeClient.setTimeOffset((TIMEZONE+DST)*3600);
EEPROM.write(adresa, timezone0);  // 0..37
EEPROM.commit();    //Store data to EEPROM 
...
if ((realDHT == 1) and (millis() - tpcitire > tppauza))
{ 
citiresenzor();
tpcitire = millis();
}
...
{
  /*
  te = random(-2000,4000)/100.;
  has = random(10000)/100.;
  */
  has = dht.getHumidity();
